...
token = os.environ["token4"]
channelid = os.environ["id4"]  # replace with channel where bot should send commands
dmid = os.environ[
    "id4"
...
async def btall():
    while True:
        ...
        await asyncio.sleep(1230)  # interval between each command in seconds


async def ho():
    while True:
        client.sendMessage(str(channelid), ".bt max")  # Command: .bt or .bt all
        await asyncio.sleep(choice([930])) 
