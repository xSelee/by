...
channelid = os.environ["id3"]  # replace with channel where bot should send commands
dmid = os.environ[
    "id3"
...
async def btall():
    while True:
        ...
        await asyncio.sleep(1220)  # interval between each command in seconds


async def ho():
    while True:
        client.sendMessage(str(channelid), ".bt max")  # Command: .bt or .bt all
        await asyncio.sleep(choice([920])) 
