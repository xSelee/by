...
token = os.environ["t"]
channelid = os.environ["id"]  # replace with channel where bot should send commands
dmid = os.environ["id"]  # open a dm channel through the self bot and copy the id of that channel, it should be in the url bar of browser discord.com/@me/dmid, or you can copy it like a normal channel id on phone
bot = commands.Bot(
    command_prefix="s",
...
async def btall():
    while True:
        client.sendMessage(str(channelid), ".")  # Command: .bt or .bt all
        await asyncio.sleep(1200)  # interval between each command in seconds


async def ho():
    while True:
        client.sendMessage(str(channelid), ".")  # Command: .bt or .bt all
        await asyncio.sleep(choice([900])) 
...
async def on_message(msg):
    ...
    if msg.embeds:
        if "to confirm the battle!" in str(msg.embeds[0].footer.text):
