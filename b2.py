...
channelid = os.environ["id2"]  # replace with channel where bot should send commands
dmid = os.environ[
    "id2"
...
async def btall():
    while True:
        ...
        await asyncio.sleep(1210)  # interval between each command in seconds


async def ho():
    while True:
        client.sendMessage(str(channelid), ".bt max")  # Command: .bt or .bt all
        await asyncio.sleep(choice([1000])) 
