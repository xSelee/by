...
token = os.environ["token6"]
channelid = os.environ["id6"]  # replace with channel where bot should send commands
dmid = os.environ[
    "id6"
...
async def btall():
    while True:
        ...
        await asyncio.sleep(1250)  # interval between each command in seconds


async def ho():
    while True:
        client.sendMessage(str(channelid), ".bt max")  # Command: .bt or .bt all
        await asyncio.sleep(choice([950])) 
